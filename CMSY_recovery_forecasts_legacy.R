...
#upload data
datafile =  "Overall_stocks_outputs.csv"#"stock1.csv" #Overall_stocks_outputs.csv"
...
SPF = function(B_Bmsy,Bmsy,Fmsy,Fcur,Fx,nyrs){ # Ft = Target F
  ...
  for(y in 1:(nyrs+1))
  {
    Fproj = ifelse(Fx <  rep(0.95,length(Fmsy)) & Pmsy[,y]<0.5,2*Pmsy[,y]*Fmsy*Fx,Fmsy*Fx)
    if(Fx == 0.5) Fproj = ifelse(Pmsy[,y]<0.5,Fmsy*0.01,Fmsy*Fx)
...
for(j in 1:length(Regs)){
  ...
  ylab= "Stocks at Bmsy (%)"
  plot(years,years,ylim=c(0,100),xlim=c(years[2],(years[nyrs]+2.3)),xaxs="i",yaxs="i",frame=FALSE,xlab="",ylab="",type="n",lwd=2,main=paste(Regs[j]), yaxt="n")
...
# Profitability #B/Bforecast_start_year
